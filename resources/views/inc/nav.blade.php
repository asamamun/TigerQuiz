    ...
    <nav class="navbar navbar-expand-lg bg-light navbar-light py-3 py-lg-0 px-0 px-lg-5">
        <div class="d-flex col-3">
            <a href="{{ url('./') }}" class="navbar-brand font-weight-bold text-secondary"style="font-size: 50px">
                <span class="icon logo"><img src="{{ url('assets/img/logo/tiger_logo.png') }}" class="" width="20%"
                        alt="logo"></span>
                ...
                @if (Route::has('login'))
                    <div class="hidden auth-nav fixed top-0 right-0 sm:block">
                        @auth
                            ...
                            @endphp
                            @if ($r == '1')
                <li class="nav-item ms-5 auth-nav"><a href="{{ url('/dashboard') }}"
                        class="text-sm text-gray dark:text-gray">Dashboard</a></li>
                @endif
                @if ($r == '2')
                    <li class="nav-item ms-5 auth-nav"><a href="{{ url('/teacher') }}"
                            class="text-sm text-gray dark:text-gray">Dashboard</a></li>
                @endif
                @if ($r == '3')
